...
* ConfigStr = "I2P n%- N%- I%-#IsoLines!s F%-PlgnOpti|PlgnFineNess!d!F f%-PllnOpti|PllnFineNess!d!F L%-Normal|Size!F k%-Override?|r,g,b|(Polyline)!d!s K%-Override?|r,g,b|(Polygon)!d!s W%-Wiresetup!d S%-x,y,z{,a,d,s}|(Light)!s p%-Point|Size!F Z%-ZMin|ZMax!F!F j%-ZOffset!F M%- V%- P%- t%- o%- w%-";
...
static void ParserSubmitPointVertex(ParserStruct* Parser, VertexStruct Vert, int IsVector)
{
    ...
    for (i = 0; i < 4; i++) {
	tx = Vectors[i][0] * Parser -> Settings.PointSize;
	ty = Vectors[i][1] * Parser -> Settings.PointSize;
	tz = Vectors[i][2] * Parser -> Settings.PointSize;
