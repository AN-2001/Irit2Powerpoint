...
#ifdef DEBUG
    #define  IRIT_VERSION	"Ver. 13D"		 /* Program version. */
#else
    #define  IRIT_VERSION	"Ver. 13"		 /* Program version. */
#endif
#define  IRIT_COPYRIGHT	"(C) Copyright 1989-2023 Gershon Elber, Technion"
...
/* The following is an integer that equal in size to pointer in the system. */
...
#if defined(LINUX386) || defined(SUN4) || defined(__MACOSX__)
#include <stdint.h>
typedef int16_t		IritInt16;
typedef int32_t		IritInt32;
#elif defined(__WINNT__)
typedef __int16		IritInt16;
typedef __int32		IritInt32;
#else
typedef int		IritInt16
typedef int		IritInt32
#endif /* LINUX386 || SUN4 */
...
#    define IRIT_FATAL_ERROR(Str)		IritMiscFatalError("")
#    define IRIT_FATAL_ERROR_PRINTF(...) 	IritMiscFatalErrorPrintf("")
...
#define IRIT_VEC_SQR_LENGTH_LEN(Pt, Len) IritGeomVecDotProdLen(Pt, Pt, Len)
...
/* The string is allocated dynamically and returned.			    */
...
#else
#define IRIT_INPUT_LINE_LEN	100000
/* The string is allocated dynamically and returned.			    */
#define IRIT_VSPRINTF(p, Format, ArgPtr) { \
    char \
        *_Line = (char *) malloc(IRIT_INPUT_LINE_LEN); \
    vsnprintf(p = _Line, IRIT_INPUT_LINE_LEN, Format, ArgPtr); \
}
