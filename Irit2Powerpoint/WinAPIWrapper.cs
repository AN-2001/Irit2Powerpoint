...
namespace Irit2Powerpoint
{
    ...
    class WindowWrapper : OpenTK.GameWindow
    {
        ...
        {
            OpenTK.Matrix4 Trans, Tmp, Proj, AspectRatioCorrector;
            OpenTK.Vector4 p0, p1;
            ...
            if (IsMoving)
            {
                ...
                AspectRatioCorrector = Renderer.TransCtx.GetAspectRatioCorrector();
                Width = Renderer.TransCtx.GetWidth();
                Height = Renderer.TransCtx.GetHeight();

                p0 = new OpenTK.Vector4((2.0f * LastMouseX / Width) - 1, 1 - (2.0f * LastMouseY / Height), 0, 1);
                p1 = new OpenTK.Vector4((2.0f * x / Width) - 1, 1 - (2.0f * y / Height), 0, 1);

                p0 = Proj.Inverted() * AspectRatioCorrector.Inverted() * p0;
                p1 = Proj.Inverted() * AspectRatioCorrector.Inverted() * p1;
                ...
                Renderer.TransCtx.SetActiveModelView(Trans * OpenTK.Matrix4.CreateTranslation(p1.Xyz));
            }
        ...
        private void handleSaveMatrices()
        {
            /* No saving if there's no directory to save in. */
            if (this.AppDirectory == null)
                return;
            string matrices = Renderer.TransCtx.getMatricesString();
            string filePath = Path.Combine(this.AppDirectory, "matrices.imd");
            try
            {
                File.WriteAllText(filePath, matrices);
            }
            catch (Exception ex)
            {
                // Handle exceptions (file locked, permission issues, etc.)
                Logger.GetInstance().Error($"Failed to save matrices: {ex.Message}");
            }
        ...
        private IntPtr WindowProc(IntPtr hWnd, uint uMsg, IntPtr wParam, IntPtr lParam)
        {
            ...
            switch (uMsg)
            {
                ...
                case WM_PAINT:
                    {
                        PAINTSTRUCT ps;
                        BeginPaint(this.hWnd, out ps);
                        this.Renderer.Render();
                        EndPaint(this.hWnd, ref ps);
                    }
                    break;
                case WM_MOUSEWHEEL:
