...
namespace Irit2Powerpoint
{
    public static class ITDParser
    {
        ...
        [DllImport("ITDParser.dll", CallingConvention = CallingConvention.Cdecl)]
        public static extern void ITDParserSetLogger(LoggerDelegate Logger);
        ...
        private static ITDMesh MarshalMesh(MeshStruct Mesh)
        {
            ...
            for (i = 0; i < Mesh.TotalVertices; i++)
                Ret.Vertecies[i] =
                    Marshal.PtrToStructure<VertexStruct>(IntPtr.Add( Mesh.Vertices, i * VERTEX_STRUCT_SIZE));
            for (i = 0; i < 32; i++)
                Ret.Lights[i] =
                    Marshal.PtrToStructure<LightStruct>(IntPtr.Add(Mesh.Lights, i * LIGHT_STRUCT_SIZE));
